...
const Play = () => {
  ...
  useEffect(() => {
    ...
    // Check if this is a popup login redirect - send message to parent window
    const popupLogin = urlParams.get("popup_login");
    if (popupLogin === "true" && window.opener) {
      // We're in a popup window after successful login
      // Send message to parent window to close popup and reload
      try {
        window.opener.postMessage(
          { type: "SHOPIFY_LOGIN_SUCCESS", timestamp: new Date().toISOString() },
  ...
  // Listen for postMessage from popup after successful login
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // Verify message is from our app domain
      const appBaseUrl =
      ...
      if (event.data && event.data.type === "SHOPIFY_LOGIN_SUCCESS") {
        ...
        // Log success
        console.log("✅ Shopify login successful! Popup closed.");
  ...
  const handleLogin = () => {
    ...
    // Use the specific app URL for return redirect
    // Add a parameter to indicate this is a popup login
    const appBaseUrl =
      import.meta.env.VITE_APP_URL || "https://id-preview--46e7a4fc-a12f-4e7f-812c-75f62bdac4d4.lovable.app";
    const returnUrl = `${appBaseUrl}/play?popup_login=true`;

    console.log("Opening login popup with return URL:", returnUrl);
    ...
    // Open login in a new popup window
    const popup = window.open(loginUrl, "ShopifyLogin", "width=500,height=700,scrollbars=yes,resizable=yes");

    if (!popup) {
      toast({
        title: "Popup Blocked",
        description: "Please allow popups for this site to login.",
        variant: "destructive",
      });
      return;
    }
    ...
    // Also check if popup is manually closed
    ...
    setTimeout(
      () => {
        ...
        if (popup && !popup.closed) {
          popup.close();
          setLoginPopup(null);
          console.log("Login popup timeout - closing window");
        }
      },
      5 * 60 * 1000,
    ); // 5 minutes
