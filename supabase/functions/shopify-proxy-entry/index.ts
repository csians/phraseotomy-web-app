...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // If no customer is logged in, show login page (or app with cookie via postMessage if cookie exists)
    if (!customerId) {
      console.log("No customer from proxy, showing login page (parent may pass cookie via postMessage)");
 ...
 * the parent can read the cookie and postMessage it to the iframe.
 */
function generateLoginRedirectHtml(loginUrl: string, shop: string, tenant: { environment: string; shop_domain: string; tenant_key: string; id: string; name: string }) {
  // Always use ourstagingserver.com for iframe when parent passes cookie via postMessage
  // (avoids loop when iframe would load phraseotomy.com and re-trigger proxy)
